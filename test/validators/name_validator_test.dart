...
void main() {
  group('NameValidator', () {
    test('Valid English name', () {
      final result = NameValidator.validate('Hero Ahmed');

      expect(result.isValid, true);
    });

    test('Valid Arabic name', () {
      final result = NameValidator.validate('محمد أحمد');
    ...
    test('Name less than minimum length', () {
      final result = NameValidator.validate('A');

      expect(result.isValid, false);
      expect(result.error, ValidationError.invalidName);
    });

    test('Name contains numbers', () {
      final result = NameValidator.validate('Hero123');
